...
#For example:
#Test: contact = create_contact("Mario Rossi", email="mario.rossi@gmail.com", telefono=788787)
...
#Result: {'profile': 'Mario Rossi', 'email': 'mario.rossi@gmail.com', 'telefono': 788787}
...
def update_contact(dictionary: dict, name: str, email: str =None, telefono: int=None) -> dict:
    for key, value in dictionary.items():
        if key == "name":
            if value != name:
                dictionary.update({key: name})
        if email:
            if key == "email":
                if value != email:
                    dictionary.update({key: email})
        if telefono:
            if key == "telefono":
                if value != telefono:
                    dictionary.update({key: telefono})
    return dictionary
